...
/* This is relying on the fact that CP/M is little-endian and the 
 * input data are big-endian */
	dest = (unsigned char *)function;
	dest[0] = (*input)[1];
	dest[1] = (*input)[0];
	(*input) += 2;	
	(*inp_len) -= 2;
...
dsk_err_t dsk_unpack_bytes(unsigned char **input, int *inp_len, unsigned char **buf)
{
	...
	if (*inp_len < (int)len) return DSK_ERR_RPC;
	*buf = *input;
